...
namespace AvyyanBackend.Services
{
    public class StorageCaptureService : IStorageCaptureService
    {
        ...
        public async Task<StorageCaptureResponseDto?> GetStorageCaptureByIdAsync(int id)
        {
            _logger.LogDebug("Getting storage capture by ID: {StorageCaptureId}", id);
            var storageCapture = await _storageCaptureRepository.GetByIdAsync(id);
            if (storageCapture == null)
            {
                _logger.LogWarning("Storage capture {StorageCaptureId} not found", id);
                return null;
            }
